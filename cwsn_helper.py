# !!!! Find ReadMe to understand what is going on and how things work !!!
...
# Init Pygame
pygame.init()
screen = pygame.display.set_mode((1920, 1080)) #can change resolution according to your needs
pygame.display.set_caption("Sign Language Game")
font = pygame.font.SysFont(None, 48)

image_folder = "sign_images"   #make this folder in same directory and store sign images to it
data_file = "student_progress.json"  # daily or session progress will be stored in this file
...
# Login.  just enter your name, it is a simple login!!
def login():
    name = pygame_login()
    data = load_data()
    if name not in data:
...
def show_and_speak(letter):
    screen.fill((0, 0, 0))
    image_path = os.path.join(image_folder, f"{letter.upper()}.jpg") #store sign images with letter name, like h.jpg for H 
...
# NormalMode --> speak word and display pic  
...
#send report to guardians, i used telegram api for it, sim module can also be used for this but i found telegram more
import requests

TELEGRAM_TOKEN = 'get telegram token and fill it here'
CHAT_ID = 'chart id can be taken from telegram'
